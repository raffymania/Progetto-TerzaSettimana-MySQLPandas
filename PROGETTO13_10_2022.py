...
print('QUERY1. OTTENGO PER OGNI CITTA LO STATO DI SPEDIZIONE')
for elem in result:
    print(elem[1:3])
conn.close()
print('\n')
...
cursor.execute(sql)
result = cursor.fetchall()
print('Query 7: CIASCUN PRODOTTO DIVISO PER CATEGORIA, IN BASE ALLA QUANTITA PRESENTE \nIN MAGAZZINO, A QUELLA ORDINATA E AL PREZZO, E LA SEGUENTE:')
for elem in result:
    elem = elem[0:4]
    print(elem)
conn.close()
...
cursor.execute(sql)
result = cursor.fetchall()
print('QUERY8. CIASCUN PRODOTTO DIVISO PER:PREZZO, STATO DI SPEDIZIONE, UTENTE CHE HA ACQUISTATO E INDIRIZZO DI SPEDIZIONE')
for elem in result:
    elem = elem[0:10]
    print(elem)
conn.close()
...
result = cursor.fetchall()
print('\n')
print('QUERY10.ACQUIRENTE CHE HA SPESO DI PIU E AL QUALE INVIARE LO SCONTO')
